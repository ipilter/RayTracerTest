...
#include "Canvas.h" // before any gl related include
...
Canvas::Canvas( const math::uvec2& textureSize
                , wxWindow* parent
                , wxWindowID id
  ...
  , mQuadSize( 1.0 * ( mTextureSize.x / static_cast<float>( mTextureSize.y ) ), 1.0 )
  ...
  , mPreviousMousePosition( 0.0f, 0.0f )
{
  try
  {
    ...
    mCameras.push_back( std::make_shared<gl::Camera>( math::vec3( -mQuadSize.x / 2.0, -mQuadSize.y / 2.0, 0.0 ) ) );
  }
...
void Canvas::UpdateTexture()
{
  try
  {
    ...
    std::stringstream ss;
    ss << "Texture update runtime: " << t.ms() << " ms";
...
gl::PixelBufferObject::sptr& Canvas::GetRenderTarget()
{
  return mPBOs.back();
}

const math::uvec2& Canvas::GetTextureSize() const
{
  return mTextureSize;
}
...
void Canvas::CreateMeshes()
{
  ...
  mMeshes.push_back( std::make_shared<gl::Mesh>( vertices, indices ) );
}
...
void Canvas::CreateTextures()
{
  ...
  mTextures.push_back( std::make_shared<gl::Texture>( glm::min( mTextureSize.x, static_cast<uint32_t>( maxTextureSize ) )
                                                      , glm::min( mTextureSize.y, static_cast<uint32_t>( maxTextureSize ) ) ) );

  const size_t pixelCount( mTextures.back()->Width() * static_cast<size_t>( mTextures.back()->Height() ) );
  const size_t byteCount( pixelCount * sizeof( uint32_t ) );

  // create PBO
  // TODO: multiple for float/triple buffering
  mPBOs.push_back( std::make_shared<gl::PixelBufferObject>() );
  mPBOs.back()->Bind();
  ...
  std::stringstream ss;
  ss << "Texture with dimensions " << mTextures.back()->Width() << "x" << mTextures.back()->Height() << " created";
  dynamic_cast<MainFrame*>( GetParent()->GetParent() )->AddLogMessage( ss.str() );
}
...
void Canvas::CreateShaders()
{
  ...
  mShaders.push_back( std::make_shared<gl::Shader>( vertexShaderSrc, fragentShaderSrc ) );
}

math::vec2 Canvas::ScreenToWorld( const math::vec2& screen )
{
...
math::ivec2 Canvas::WorldToImage( const math::vec2& worldSpacePoint )
{
  const float x( worldSpacePoint.x / mQuadSize.x * mTextures.back()->Width() );
  const float y( -worldSpacePoint.y / mQuadSize.y * mTextures.back()->Height() + mTextures.back()->Height() );  // texture`s and world`s y are in the opposite order

  if ( x < 0.0f || x >= static_cast<float>( mTextures.back()->Width() ) 
       || y < 0.0f || y >= static_cast<float>( mTextures.back()->Height() ) )
  {
